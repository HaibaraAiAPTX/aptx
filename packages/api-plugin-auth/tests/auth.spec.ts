...
import { createAuthController, createAuthMiddleware, SKIP_AUTH_REFRESH_META_KEY } from "../src/index.js";
...
describe("auth middleware", () => {
  ...
  it("should skip auth refresh when meta.skipAuthRefresh is true", async () => {
    ...
    const mw = createAuthMiddleware({
      ...
      refreshToken: async () => {
        refreshCalled = true;
        return { token: "t1", expiresAt: Date.now() + 3600000 };
      },
    });

    // 模拟带有 skipAuthRefresh 标记的请求返回 401
    await expect(
      mw.handle(
        new Request({ method: "GET", url: "https://example.com", meta: { [SKIP_AUTH_REFRESH_META_KEY]: true } }),
        new Context({ id: "1", signal: new AbortController().signal }),
        async () => {
          throw new HttpError("HTTP 401", 401, "https://example.com");
        },
  ...
  it("should attempt refresh when meta.skipAuthRefresh is false or undefined", async () => {
    let refreshCalled = false;
    const store: TokenStore = {
      getToken: () => "t0",
      setToken: () => {},
    ...
    let callCount = 0;
    await mw.handle(
      new Request({ method: "GET", url: "https://example.com", meta: {} }),
      new Context({ id: "1", signal: new AbortController().signal }),
      async () => {
        callCount++;
        if (callCount === 1) {
